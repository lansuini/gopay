...
var mainConfigFile = flag.String("config", "./config.yaml", "配置文件路径")

func init() {
	flag.Parse()

	// 初始化配置
	conf := config.Init(*mainConfigFile)
	dateTimeFormat := utils.GetFormatTime(time.Now())
	logFile := conf.LogPath + "main-" + dateTimeFormat + ".log"
	// 初始化日志
