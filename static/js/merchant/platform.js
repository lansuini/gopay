$(function() {
    common.getAjax(apiPath + "basedata?requireItems=commonStatus,openType", function(a) {
        apiPath += "merchant/";
        ...
        $("#btnSearch").initSearch(apiPath + "platformsearch", getColumns());
    });
...
function showSignKeyModal(a) {
    common.getAjax(apiPath + "signkey?platformNo=" + a, function(b) {
...
function showEditModal(a) {
    common.getAjax(apiPath + "platformdetail?platformNo=" + a, function(b) {
        if (b.success) {
            $("#txtPlatformNo").val(a);
...
function submit() {
    ...
    common.submit(apiPath + "platformupdate?domains=" + encodeURI($("#txtDomains").val().replace(/\n/g, "|")), "editModal", function() {
