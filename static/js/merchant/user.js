$(function() {
    common.getAjax(apiPath + "basedata?requireItems=merchantUserLevel,merchantUserStatus", function(a) {
        ...
        $("#btnSearch").initSearch(apiPath + "merchant/usersearch", getColumns())
    });
    ...
    return [{
        ...
        formatter: function(b, c, a) {
            str = "<a onclick='showEditModal(" + JSON.stringify(c) + ")'>修改</a>" +
                "<a onclick='resetPwd(\"" + c.accountId + "\", true)'>重置登录密码</a>" +
                "<a onclick='resetPwd(\"" + c.accountId + "\", false)'>重置支付密码</a>"
            if(c.googleAuthSecretKey != '')
                str = str + "<a onclick='googleAuthSecretKey(\"" + c.accoountId + "\")'>关闭登陆谷歌验证</a>"
            return str
...
function resetPwd(b, a) {
    ...
    myConfirm.show({
        title: "您确定要重置" + c,
        sure_callback: function() {
            common.getAjax(apiPath + "merchant/" + d + "?userId=" + b, function(e) {
                if (e && e.success) {
...
function submit() {
    ...
    common.submit(apiPath + "merchant/userupdate", "editModal", function() {
        location.href = location.href
    })
}
;