var payOrderStatus,payType
$(function() {
    common.getAjax(apiPath + "basedata?requireItems=payOrderStatus,payType", function(a) {
        payOrderStatus = a.result.payOrderStatus
...
function getColumns() {
    return [{
        ...
        align: "center",
        formatter: function(b,c,a) {
            payOrderStatus.forEach(function(item){
                if(item.key === c.orderStatus){
                    b = item.value
                    return item.value
                }
            });
        ...
        align: "center",
        formatter: function(b, c, a) {
            return c.orderStatus == "Success" ? "<a onclick='sendNotify(\"" + c.platformOrderNo + "\")'>补发通知</a>" : ""
        }
    }]
}
...
function sendNotify(a) {
    common.submit(apiPath + "payorder/notify?platformOrderNo=" + a)
}
;