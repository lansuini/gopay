$(function() {
    common.getAjax(apiPath + "basedata?requireItems=checkStatusCode,bankrollType,bankrollDirection", function(a) {
...
function showPwdModal(a) {
    ...
    if(a.type == "余额调整") {
        ...
        $("#selBankrollType").val(content.bankrollType);
        $("#selBankrollDirection").val(content.bankrollDirection);
        // 充值显示系统手续费和实际手续费
        if(content.bankrollDirection == 'Recharge'){
            $("#sysFee").val(content.sysFee);
            $("#factFee").val(content.factFee);
            $('#div-sys-fee').show();
            $('#div-fact-fee').show();
        }
        $("#txtAmount").val(content.amount);
        $("#txtSummary").val(content.summary);
        ...
        $("#withdrawbank").val(content.bankName);
        $("#withdrawname").val(content.userName);
        $("#withdrawcard").val(content.cardNo);
        $("#withdrawmoney").val(content.issueAmount);
        $("#withdrawdesc").val(content.desc);
    } else {
    ...
    $("#newpassword").val(content.password);
    $(".pwdButton").hide();
    if (a.status == "待审核") {
        $(".pwdButton").show();
    }
...
function pwdBtnSubmit() {
    ...
    if(passwordtype == "余额调整") {
        var action = method == "resetpassword" ? "balance/audit" : "balance/unaudit";
        var url = "check/" + action + "?id=" + id + "&auditType=" + passwordtype;
...
function showEditModal(a) {    
    ...
    if(a.type == '支付补单' || a.type == '代付补单'){
        ...
        $("#editModal #orderId").val(content.orderId);
        $("#editModal #platformOrderNo").val(content.platformOrderNo);
        $("#editModal #orderAmount").val(content.orderAmount);
        $("#editModal #channel").val(content.channel);
        $("#editModal #channelMerchantNo").val(content.channelMerchantNo);
        $("#editModal #channelOrderNo").val(content.channelOrderNo);
        $("#editModal #channelNoticeTime").val(content.channelNoticeTime);
        $("#editModal #selOrderStatus").val(content.orderStatus);
        $("#editModal #desc").val(content.desc);
        $("#editModal .type").val(a.type);
        if(content.pic != undefined){
            $("#editModal #pic").attr('src','data:image/png;base64,'+content.pic);
        } else {
...
function showEditSoModal(a) {    
    ...
    if(a.type == '代付补单'){
        ...
        $("#editSoModal #orderId").val(content.orderId);
        $("#editSoModal #platformOrderNo").val(content.platformOrderNo);
        $("#editSoModal #orderAmount").val(content.orderAmount);
        $("#editSoModal #channel").val(content.channel);
        $("#editSoModal #channelMerchantNo").val(content.channelMerchantNo);
        $("#editSoModal #channelOrderNo").val(content.channelOrderNo);
        $("#editSoModal #channelNoticeTime").val(content.channelNoticeTime);
        $("#editSoModal #selOrderStatus").val(content.orderStatus);
        ...
        $("#editSoModal #desc").val(content.desc);
        $("#editSoModal .type").val(a.type);
        if(content.failReason != ''){
            $("#editSoModal #txtFailReason").val(content.failReason);
            $("#txtFailReason").parent().show()
        }
        if(content.pic != undefined){
            $("#editSoModal #pic").attr('src','data:image/png;base64,'+content.pic);
        } else {
