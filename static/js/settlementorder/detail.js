...
function buildOrder(b) {
    ...
    a.push({
        ...
        value: common.timeFormatSeconds( b.applyTime)
    });
    a.push({
        key: "平台订单号",
    ...
    a.push({
        key: "订单创建时间",
        value: common.timeFormatSeconds(b.created_at)
    });
...
function buildCheck(b) {
    ...
    a.push({
        key: "审核时间",
        value: common.timeFormatSeconds(b.auditTime)
    });
    a.push({
        key: "审核人IP",
        value: b.auditIp
...
function buildChannel(b) {
    ...
    a.push({
        key: "向上游推送时间",
        value: common.timeFormatSeconds(b.pushChannelTime)
    });
    a.push({
        key: "上游账户",
    ...
    a.push({
        key: "上游处理时间",
        value: common.timeFormatSeconds(b.channelNoticeTime)
    });
    a.push({
        key: "处理标识",
...
function submitPerfect() {
    ...
    orderStatus = $("#selOrderStatus").val()
    ...
    $.ajax({
        ...
        dataType:'json',
        type:"post",
        data: {"orderId":orderId,"type":"in","channelOrderNo":channelOrderNo,"channelNoticeTime":channelNoticeTime,"desc":desc,"orderStatus":orderStatus,"failReason":failReason},
        async:false,
        success:function (d) {
            if (d) {
                if (d.success == -1) {
