...
function bindData(a) {
    ...
    if(type == 'info'){
        ...
        $("#check_time").val(common.timeFormatSeconds(a.check_time));
        $("#desc").val(a.desc);
    ...
    $("#selOrderStatus").val(content.orderStatus);
    $("#commiter_desc").val(content.desc);
    $("#ip").val(a.ip);
    $("#created_at").val(common.timeFormatSeconds(a.created_at));
    // $("#pic").attr('src','data:image/png;base64,'+content.pic);
    // $("#pic").attr('onclick','javascript:showimage("'+content.pic+'")');
    if(content.failReason != ''){
        $("#txtFailReason").val(content.failReason);
