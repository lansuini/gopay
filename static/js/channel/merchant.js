$(function() {
    common.getAjax(apiPath + "basedata?requireItems=channel,commonStatus,switchType,settlementType", function(a) {
    ...
    $("#selChannel").change(function(b){
        ...
        $('#merchant_param').html("");
        common.getAjax(apiPath+'parameter?name='+channelVal, function(a) {
            var res = a.result;
            var desc = a.desc;
            spellHtml(res,desc);
        });
...
function getColumns() {
    ...
    },
    {
        ...
        align: "center",
        formatter: function (b, c, a) {
            var e = '<label id="txtBalance'+ c.merchantNo +'">'+ c.balance + '</label>'+ '<br/>' + '<label type="hidden" data-toggle="tooltip" data-placement="top" id="balance'+ c.merchantNo + '"></label>'+ '<a href="javascript:;" onclick="queryBalance(\'' + c.merchantNo + '\')">刷新</a> ';
            return e ;
        }
    },{
        field: "openPay",
...
function batchStatus(btnid, status) {
    ...
    common.postAjax(apiPath + "batchupdatestatus?status=" + status + "&ids=" + ids, function(a) {
        if (a.success){
...
function queryBalance(merchantNo){
    $.ajax({
        type: 'GET',
        url: apiPath + 'balance?merchantNo=' + merchantNo,
        async: false,

        success: function (d) {

            if(d.status == 'Success'){
                $("#txtBalance"+merchantNo).html(d.balance);
                showPopover($("#balance"+merchantNo),d.balance);
            }else{
                showPopover($("#balance"+merchantNo),d.failReason);
            }
...
function balanceSubmit(b) {
    ...
    $.ajax({
        ...
        dataType:'json',
        type:"post",
        data:common.getFields("addBalance"),
        success:function (d) {
            if (d) {
                if (d.success == -1) {
